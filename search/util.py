...
def evaluation_function(state: C4State):
    """
    ...
    against = feature_2(state, state.last_player)
    if against == float('inf'):
        return -against
    
    # if it leads to an automatic winning position
    in_favour = feature_2(state, state.last_player)
    if in_favour == float('inf'):
        return in_favour

    in_favour += feature_3(state, state.last_player) + feature_4(state, state.last_player)
    against += feature_3(state, 3 - state.last_player) + feature_4(state, 3 - state.last_player)
